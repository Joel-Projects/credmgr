...
class Session(object):
    ...
    def _do_retry(self, data, files, json, method, params, response, retry_strategy_state, saved_exception, url, ):
        if saved_exception:
    ...
    def _make_request(self, data, files, json, method, params, retry_strategy_state, url):
        try:
            ...
            if not retry_strategy_state.should_retry_on_failure() or not isinstance(exception.original_exception, self.RETRY_EXCEPTIONS):
                raise
            return None, exception.original_exception

    def _request_with_retries(self, data, files, json, method, params, url, retry_strategy_state=None):
        ...
        response, saved_exception = self._make_request(data, files, json, method, params, retry_strategy_state, url)
        ...
        if retry_strategy_state.should_retry_on_failure() and (do_retry or response is None or response.status_code in self.RETRY_STATUSES):
            return self._do_retry(data, files, json, method, params, response, retry_strategy_state, saved_exception, url)
