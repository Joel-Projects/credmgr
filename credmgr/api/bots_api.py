...
class BotsApi(object):
    ...
    def get_bots_with_http_info(self, **kwargs):
        ...
        **PERMISSIONS: At least Active user is required.**   Returns a list of Bots starting from ``offset`` limited by ``limit`` parameter.  Only Admins can specify ``owner`` to see Bots for other users. Regular users will see their own Bots.
        ...
        if 'limit' in params and params['limit'] < 1:
            raise ValueError("Invalid value for parameter `limit` when calling `get_bots`, must be a value greater than or equal to `1`")
        if 'offset' in params and params['offset'] < 0:
            raise ValueError("Invalid value for parameter `offset` when calling `get_bots`, must be a value greater than or equal to `0`")
        collection_formats = {}
    ...
    def post_bots(self, app_name, **kwargs):
        ...
        **PERMISSIONS: At least Active user is required.**   Bots are used for grouping apps into a single request
        ...
        :param int reddit_app_id: Reddit App the bot will use
        :param int sentry_token_id: Sentry Token the bot will use
        :param int database_credential_id: Database Credentials the bot will use
    ...
    def post_bots_with_http_info(self, app_name, **kwargs):
        ...
        **PERMISSIONS: At least Active user is required.**   Bots are used for grouping apps into a single request
        ...
        all_params = ['name', 'reddit_app_id', 'sentry_token_id', 'database_credential_id', 'owner_id']
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
        ...
        if 'sentry_token_id' in params:
            form_params.append(('sentry_token_id', params['sentry_token_id']))
        if 'database_credential_id' in params:
            form_params.append(('database_credential_id', params['database_credential_id']))
        if 'owner_id' in params:
